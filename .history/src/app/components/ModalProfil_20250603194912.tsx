...
import { Users, Star, Briefcase, X, ExternalLink, Mail, Calendar, MapPin, Award, Loader2, LinkIcon, UserPlus, UserMinus, User } from 'lucide-react'
import { FaLinkedin } from "react-icons/fa"

import { ModalAvis } from "./ModalAvis"
import { ModalAbonnes } from "./ModalAbonnes"
importModalAbonnements } from "./ModalAbonnements"
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 overflow-y-auto px-4 py-6">
      <div className="bg-white/90 backdrop-blur-xl rounded-3xl w-full max-w-md max-h-[90vh] shadow-2xl border border-white/20 relative overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-slate-300">
        ...
        {showAbonnementsModal && (
          <ModalAbonnements userId={userId} onClose={() => setShowAbonnementsModal(false)} />
        )}

        {showAbonnesModal && (
          <ModalAbonnes userId={userId} onClose={() => setShowAbonnesModal(false)} />
        )}

        {showAvisModal && (
          <ModalAvis userId={userId} onClose={() => setShowAvisModal(false)} />
        )}

        {showMissionsModal && (
          <ModalMissions userId={userId} onClose={() => setShowMissionsModal(false)} />
        )}
      </div>
    </div>
...
function Stat({
  ...
  return (
    <div
      onClick={onClick}
      className={`flex flex-col items-center p-3 rounded-xl bg-gradient-to-r ${bgColor} border ${borderColor} hover:shadow-md transition-all cursor-pointer`}
    >
