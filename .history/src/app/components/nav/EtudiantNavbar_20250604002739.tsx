...
export default function EtudiantNavbar({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex">
      <aside className="fixed top-0 left-0 h-screen w-64 shadow-lg bg-white z-50 flex flex-col"> 
        {/* Logo */}
        <div className="p-6"> 
        ...
        {/* Navigation */}
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto shadow-inner">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
        ...
        {/* Profil */}
        <div className="p-4 shadow-inner "> {/* Bordure */}
          <Link
