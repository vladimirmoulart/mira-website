...
export default async function Navbar({ pathname }: { pathname: string }) {
  ...
  const userNom = data?.nom || "Utilisateur"
  const userAvatar = data?.avatar || ""
  const userRole = data?.role === 1 ? "Étudiant" : data?.role === 2 ? "Entreprise" : "Administrateur"

  const navItems = [
    { icon: LayoutDashboard, label: "Fil d'actualité", href: "/dashboard/feed" },
    { icon: Briefcase, label: "Mes missions", href: "/dashboard/mes-missions", badge: "Étudiant" },
    { icon: Building2, label: "Mes projets", href: "/dashboard/mes-projets", badge: "Entreprise" },
    { icon: Search, label: "Trouver une mission", href: "/dashboard/rechercher-une-mission", badge: "Étudiant" },
  ...
  return (
    <aside className="fixed top-0 left-0 h-screen w-70 bg-gradient-to-b from-slate-50/95 to-white/95 backdrop-blur-xl border-r border-slate-200/60 shadow-xl z-50 flex flex-col">
      {/* Header with Logo */}
      <div className="p-8 border-b border-slate-200/50">
        <Link
          href="/"
          className="flex items-center justify-center group transition-transform duration-200 hover:scale-105"
        >
      ...
      <nav className="flex-1 p-6 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent">
        {navItems
          .filter((item) => !item.badge || item.badge === userRole)
          .map((item) => (
            <Link
              key={item.href}
              href={item.href}
