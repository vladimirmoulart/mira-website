...
export default function MissionDetailPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    const res = await fetch(`/api/mes-missions/${id}`)
    if (!res.ok) {
    ...
    setPostulationStatus(data.postulationStatus)
    setLoading(false)
  }
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-6">
              <div className="flex-1">
                <h1 className="text-3xl font-bold bg-[#52c1ff] bg-clip-text text-transparent mb-4">
                  {mission.titre}
                </h1>
            ...
            <div className="flex border-b border-gray-200/50">
              <button
                ...
                className={`flex-1 px-6 py-4 text-sm font-medium transition-all duration-300 flex items-center justify-center gap-2 ${
                  activeTab === "details"
                    ? "bg-[#52c1ff]/10  text-[#52c1ff] border-b-2 border-[#52c1ff]"
              ...
              <button
                ...
                className={`flex-1 px-6 py-4 text-sm font-medium transition-all duration-300 flex items-center justify-center gap-2 ${
                  activeTab === "chat"
                    ? "bg-[#ffbb88]/10 text-[#ffbb88] border-b-2 border-[#ffbb88]"
