...
export default function RechercherMission() {
  ...
  const [postulationStatus, setPostulationStatus] = useState<Record<number, "loading" | "success" | "error">>({})
  ...
  const handlePostuler = async (mission: Mission) => {
    setPostulationStatus((prev) => ({ ...prev, [mission.id]: "loading" }))
    ...
    try {
      ...
      if (!user) {
        ...
        setPostulationStatus((prev) => ({ ...prev, [mission.id]: "error" }))
        return
      }

      // Vérifier si l'utilisateur a déjà postulé
      const { data: existingPostulation } = await supabase
        .from("postulations")
        .select("id")
      ...
      if (error) {
        ...
        setPostulationStatus((prev) => ({ ...prev, [mission.id]: "success" }))
        setTimeout(() => {
          setPostulationStatus((prev) => ({ ...prev, [mission.id]: undefined }))
        }, 3000)
      }
    } catch (error) {
              ...
              {filteredMissions.map((mission, index) => {
                ...
                return (
                  <MissionCard
                    ...
                    postulationStatus={postulationStatus[mission.id]}
                    index={index}
                  />
                )
              })}
...
function MissionCard({
  ...
  postulationStatus?: "loading" | "success" | "error"
  index: number
}) {
  const gradients = [
...
<button
  ...
  disabled={postulationStatus === "loading" || postulationStatus === "success"}
  className={`w-full py-3 rounded-xl font-semibold text-sm transition-all duration-300 flex items-center justify-center gap-2 ${
    postulationStatus === "success"
      ? "bg-[#ffbb88] text-white"
      : postulationStatus === "error"
        ? "bg-red-500 text-white"
        : "bg-[#52c1ff] hover:from-[#52c1ff]/90 hover:to-[#ffbb88]/90 text-white hover:shadow-lg"
  }`}
>
  {postulationStatus === "loading" && <Loader2 className="w-4 h-4 animate-spin" />}
  {postulationStatus === "success" && <CheckCircle className="w-4 h-4" />}
  {postulationStatus === "error" && <AlertCircle className="w-4 h-4" />}
  {postulationStatus === "success"
    ? "Candidature envoyée !"
    : postulationStatus === "error"
      ? "Erreur - Réessayer"
      : postulationStatus === "loading"
        ? "Envoi en cours..."
        : "Postuler"}
</button>
