...
export default function RechercherMission() {
  ...
  const [postulationsLoaded, setPostulationsLoaded] = useState(false)
  const { showNotification, Notification } = useNotification()

  const allSkills = useMemo(() => {
    return Array.from(new Set(missions.flatMap((mission) => mission.competences || [])))
  }, [missions])

  // Consolidated filtering logic
  const displayedMissions = useMemo(() => {
    if (!postulationsLoaded) return []

    return missions.filter((mission) => {
      // Filter out completed missions and accepted applications
      if (mission.statut === "terminée" || mesPostulations[mission.id] === "acceptée") {
        return false
      }
  ...
  }, [missions, searchTerm, selectedSkill, mesPostulations, postulationsLoaded])
